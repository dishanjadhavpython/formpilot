...
import { decodeToCanvas } from './image.js';
import { preprocessForOcr, describePreprocessing } from './preprocess.js';
...
/**
 * Clean a photo up before the engine sees it.
 *
 * Separate from recognise() so the caller can report what was done and offer to
 * turn it off - and so a failure to clean up degrades to "recognise the
 * original" rather than to "recognise nothing".
 *
 * The decode is lib/image.js's, deliberately: it applies the EXIF orientation
 * tag, and OCR on a sideways card reads nothing at all.
 *
 * @param {File|Blob} file
 * @param {{deskew?:boolean, contrast?:boolean, upscale?:boolean}} options
 * @returns {Promise<{image:*, applied:object|null, note:string|null}>}
 */
export async function prepareForOcr(file, options = {}) {
  try {
    const decoded = await decodeToCanvas(file);
    const { canvas, applied } = preprocessForOcr(decoded, options);
    return { image: canvas, applied, note: describePreprocessing(applied) };
  } catch {
    // Anything unexpected here - an exotic format, a canvas the browser refuses
    // to read back - must not cost the user their OCR run. The engine can
    // decode the original file perfectly well on its own.
    return { image: file, applied: null, note: null };
  }
}
