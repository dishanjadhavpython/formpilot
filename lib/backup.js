...
/**
 * Check one encrypted vault record: the shape, the algorithm, the work factor
 * and the sizes. Used on an imported backup, and again on the record already in
 * storage before a passphrase is ever fed to it - the file on disk is no more
 * trustworthy than the file on the USB stick, it just arrived earlier.
 *
 * The wording says "backup" throughout because that is where a bad record comes
 * from in practice, and the alternative is two copies of every message.
 *
 * @returns {string|null} a human-readable problem, or null if the record is sound
 */
export function validateVaultRecord(record) {
  if (!record || typeof record !== 'object' || Array.isArray(record)) {
    return 'The backup has no vault record in it.';
  }
...
/**
 * Check a parsed backup FILE before it is allowed anywhere near real storage.
 * Writing a malformed record would leave the vault permanently unopenable, so
 * this refuses anything it cannot fully account for.
 *
...
export function validateBackup(parsed) {
  ...
  return validateVaultRecord(parsed.record);
}
