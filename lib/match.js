...
globalThis.FormPilotMatch = globalThis.FormPilotMatch || (() => {
  ...
  const SYNONYMS = {
    ...
    // Choice fields. These exist for radio groups and dropdowns - the controls
    // Indian portal forms are dense with - and are restricted to those by
    // CHOICE_ONLY below.
    gender:        ['gender', 'sex'],
    category:      ['reservation category', 'social category', 'caste category',
                    'category'],
    maritalStatus: ['marital status', 'marital']
  };

  // Keys whose value only means anything against a fixed set of options.
  //
  // "OBC" typed into a free-text box labelled "Job Category" is noise at best
  // and wrong at worst, and `category` is a generic enough word to hit boxes
  // that have nothing to do with reservation status. Restricting these to a
  // <select> or a radio group makes a bad guess harmless: the value has to
  // match one of the offered options or nothing is filled at all.
  const CHOICE_ONLY = new Set(['gender', 'category', 'maritalStatus']);

  // --- Document (file-upload) synonyms ---------------------------------------
  // Separate from SYNONYMS on purpose: these only ever run against
  // input[type=file] candidates (content.js's fileCandidates(), never
  // candidates()), so there is no risk of a text field and a file field
  // fighting over the same phrase even where the wording overlaps ("PAN
  // card"). Keyed by the same type ids options.js's DOC_GROUPS already uses -
  // reuse the vault's existing vocabulary rather than inventing a second one.
  const DOC_SYNONYMS = {
    photo:     ['passport photo', 'passport size photo', 'recent photograph',
                'photograph', 'photo id', 'upload photo', 'photo'],
    signature: ['signature', 'sign here', 'upload signature',
                'digital signature', 'scanned signature', 'signature specimen'],
    pan:       ['pan card image', 'pan card copy', 'upload pan card',
                'pan card', 'copy of pan'],
    aadhaar:   ['aadhaar card', 'aadhar card', 'aadhaar card image',
                'copy of aadhaar', 'aadhaar', 'aadhar', 'uidai card'],
    idProof:   ['id proof', 'identity proof', 'proof of identity',
                'government id', 'govt id', 'photo id proof']
  };
  ...
  function buildDictionary(customFields = [], emails = []) {
    ...
    // Only the LABEL is read here, never the value - so this accepts the
    // label-only metadata that describeVault() produces for the content script.
  ...
  /** The document-type dictionary, in the same {key, synonyms, priority} shape
   * buildDictionary() produces - so inferKey() and describeField() are reused
   * unchanged for file inputs. No per-vault customisation (no custom doc
   * types yet), so this needs no arguments. */
  function buildDocDictionary() {
    return Object.entries(DOC_SYNONYMS).map(([key, synonyms]) => ({ key, synonyms, priority: 0 }));
  }

  /**
   * Would a file input with this `accept` attribute take one of our stored
   * document images? No `accept` at all means "anything", so that is a pass.
   * Vault documents are always images (options.js refuses anything else at
   * add-time), so a field pinned to `.pdf` or similar correctly fails this -
   * there is nothing in the vault that would satisfy it.
   */
  function acceptsMime(acceptAttr, mime) {
    const accept = String(acceptAttr ?? '').trim().toLowerCase();
    if (!accept) return true;
    if (!mime) return false;

    const tokens = accept.split(',').map((t) => t.trim()).filter(Boolean);
    if (tokens.includes('image/*') || tokens.includes(mime)) return true;

    const sub = mime.split('/')[1] ?? '';
    const ext = `.${sub === 'jpeg' ? 'jpg' : sub}`;
    if (tokens.includes(ext)) return true;
    if (ext === '.jpg' && tokens.includes('.jpeg')) return true;
    return false;
  }

  /**
   * Which of these options does `value` mean?
   *
   * Used for both <select> options and the radios in a group, so the two cannot
   * disagree about what "Male" or "OBC" resolves to.
   *
   * Exact match on the option's value or its visible text, then a looser pass
   * on the text - and that second pass is WORD-BOUNDARY matched, not substring.
   * Substring matching is subtly, dangerously wrong here:
   *
   *     'female'.includes('male')   // true
   *
   * which would tick "Female" for a user who stored "Male". Anything that fills
   * one option out of a fixed set has to be exact about the boundary.
   *
   * @param {string} value
  ...
  function chooseOption(value, options = []) {
    ...
    for (const [i, option] of options.entries()) {
      ...
      if (optionValue === want || optionText === want) return i;
    }

    // "General" should still find "General / Unreserved". Two characters is too
    // short to be a safe partial ("SC" would hit "SC/ST" and "OBC" alike), so
    // the loose pass only runs on something long enough to be meaningful.
    if (want.length > 2) {
      const boundary = new RegExp(`\\b${want.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`);
      for (const [i, option] of options.entries()) {
  ...
  /**
   * The SAFE half of the document list, mirroring describeVault(): which
   * types the vault holds an image for, and the mime each one is - never the
   * image data itself. A mime type is metadata, not a secret, the same
   * category as the labels describeVault() already lets through.
   */
  function describeDocs(documents = []) {
    const docKeys = [];
    const docMimes = {};
    for (const doc of documents) {
      if (!doc?.type || !doc?.dataUrl) continue;
      if (!docKeys.includes(doc.type)) docKeys.push(doc.type);
      docMimes[doc.type] = doc.mime;
    }
    return { docKeys, docMimes };
  }

  /**
   * Narrow the document list down to the types a plan actually needs, one
   * document per type - mirrors pickValues() for the file-upload case.
   */
  function pickDocs(documents = [], types) {
    const wanted = new Set(types ?? []);
    const out = {};
    for (const doc of documents) {
      if (!doc?.type || !doc?.dataUrl || out[doc.type] || !wanted.has(doc.type)) continue;
      out[doc.type] = { name: doc.name, mime: doc.mime, dataUrl: doc.dataUrl };
    }
    return out;
  }
  ...
  /**
   * The SAFE half of expandValues: which keys the vault can answer, and the
   * labels needed to match them - with every actual value stripped out.
   *
   * This is what a web page's content script is allowed to see. Counting
   * fillable fields needs to know that `phone` HAS a value, never what it is,
   * so detection runs entirely on this and real values only ever cross into a
   * page for the specific fields a fill is about to write.
   *
   * Labels do cross ("Work", "Employee ID"), because matching cannot work
   * without them. They are field names, not personal data - the same category
   * as siteMappings.
   */
  function describeVault(fields = {}, customFields = [], emails = []) {
    const values = expandValues(fields, customFields, emails);
    return {
  ...
  /**
   * Narrow a full value map down to the keys a plan actually needs, so a fill
   * hands the page the three values it is about to type and nothing else.
   */
  ...
  // Every part of this comes from the page, and a hostile or just badly built
  // one can make it enormous - a <label> wrapping half the document, or an
  // aria-labelledby pointing at the body. Each part and the total are capped so
  // scanning stays linear in the number of fields, not in the size of the page.
  const MAX_PART = 200;
  const MAX_HAYSTACK = 600;
  ...
  function describeField(el) {
    ...
    const text = parts
      .filter(Boolean)
      .map((part) => String(part).slice(0, MAX_PART))
      .join(' ')
  ...
  return {
    AUTOCOMPLETE_MAP, SYNONYMS, NEVER, THIRD_PARTY, EMAIL_LABEL_SYNONYMS, DOC_SYNONYMS,
    CHOICE_ONLY,
    normalise, buildDictionary, expandValues, describeVault, pickValues,
    inferKey, describeField, fromAutocomplete, chooseOption,
    buildDocDictionary, acceptsMime, describeDocs, pickDocs
  };
})();
